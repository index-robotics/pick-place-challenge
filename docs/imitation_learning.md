...
back through `DemoImageDataset` (`src/pick_place_challenge/image_dataset.py`).
...
- `--res N` (collect) — square camera resolution, recorded in metadata so train/eval
  match. Larger frames cost more to decode.
...
> **Heads-up: the image dataloader is deliberately naive.** `DemoImageDataset`
> **re-decodes an episode's entire mp4 on every sample fetch** — wildly redundant and
> slow, and the bottleneck for image training. Speeding it up is the challenge; see the
> [README](../README.md).
