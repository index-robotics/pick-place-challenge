...
Deliberately tiny — the interesting part of the challenge is elsewhere, not the
learning algorithm. The MLP maps one observation to the next ``chunk`` actions; at
rollout we execute a chunk open-loop before predicting again. Chunking lets the
    ...
    torch.save(
        {
            ...
            "stats": vars(stats),
            "meta": meta,  # obs_dim, act_dim, chunk, out_dim, hidden
        },
        path,
    )
