"""Smoke tests for the imitation-learning pipeline: expert + BC.

These build a tiny env and step it, so they need MuJoCo-Warp (CPU backend is fine;
a GPU is faster). The scripted expert and the BC policy are exercised end-to-end on
2 envs for a handful of steps — enough to catch wiring breakage without a full
training run.
"""
...
def test_expert_steps_env() -> None:
    """The env builds with action dim 8 and the scripted expert drives it."""
    env = _env()
    try:
...
def test_episode_io_roundtrip(tmp_path) -> None:
    ...
    episode_io.write_metadata(ep, {"num_steps": 10})
    ...
    assert episode_io.read_metadata(ep)["num_steps"] == 10
...
def test_run_dir(tmp_path) -> None:
    ...
    run = run_dir.new_run_dir("eval", root=str(tmp_path / "exp_local"))
